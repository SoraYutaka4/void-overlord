...
import { hasItemInInventory } from "../utils/command";

const getRandomVideo = async (urls: string[]) => {
  const randomUrl = urls[Math.floor(Math.random() * urls.length)];
    try {
      ...
      console.log(`Lỗi khi tải video từ ${randomUrl}. Đang thử video khác...`);
      return getRandomVideo(urls);
    }
};
...
export default {
    info: {
        name: "cosplay",
        description: "Xem video cosplayer 🔥🎥",
        version: "1.0.0",
        prefix: true,
        aliases: ["xemcosplay", "gaicosplay", "cosplayer", "girlcosplay"],
        cooldown: 7000,
        category: ["Fun"],
        credits: "NPK31"
    },
  
    execute: async ({api, message, admin}) =>{
      try {
        if (!hasItemInInventory(message.senderID, "royal_card", 1) && !admin.is(message.senderID)) {
          return api.sendMessage("❌ Bạn không đủ điều kiện để xem lệnh này! Cần có *royal_card*.", message.threadID);
        }

        const videoStream = await getRandomVideo(link);
  
        return api.sendMessage({ attachment: [videoStream] }, message.threadID);
      } catch (error) {
        return api.sendMessage("😅 Ôi, có lỗi xảy ra khi lấy video. Thử lại sau nhé! 🔄", message.threadID);
      }
    },
